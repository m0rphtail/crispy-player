            ...
            for i in range(0, num_features, 4):
                ...
                templist.append(
                    (
                        "direct",
                        (
                            formatted_name + (str(bind_i) if bind_i > 0 else ""),
                            bind_len,
                        ),
                    )
                )
...
def get_binds_split(node, max_bind_num=16):
    ...
    for i in range(len(node)):
        ...
        real_max_bind_num = (
            max_bind_num if len(splits) == 0 else max_bind_num - 1
        )  # Splitting needs to reserve one bind for the shader itself
        if (i + 1) == len(node):
...
def parse_expression_recur(node, with_offset=False):
    ...
    if name == "add":
        return (
            "("
            + ("+".join([parse_expression_recur(n, with_offset) for n in subnode]))
            + ")"
        )
        ...
        return (
            "("
            + ("max(" + parse_expression_recur(subnode, with_offset) + ", 0.0)")
            + ")"
        )
...
def parse_expression(node, feature_no, with_offset=False):
    if with_offset:
        return (
            ...
            + str(feature_no)
            + "(x_off, y_off) "
            + parse_expression_recur(node, with_offset)
        )
    else:
        return (
            "#define g_"
...
def parse_conv2d(
    layer, hook="MAIN", desc=None, when=None, dtype=np.float32, max_bind_num=16
):
    shader_string = ""

    # Get weights and biases if exists
    weights = np.transpose(
        np.array(layer.weights[0]), (1, 0, 2, 3)
    )  # Correlation -> Convolution transpose
    ...
    for n in range(0, output_size, 4):
        ...
        for bi, binds in enumerate(bind_split_list):
            ...
            if (
                current_bind_name not in ["LUMA", "NATIVE", "RGB", "MAIN"]
                and "lastresid" not in layer.name
            ):
            ...
            for k, bind in enumerate(binds):
                ...
                for i, j in np.ndindex((kernel_height, kernel_width)):
                    ...
                    # Compute offset
                    tex_off = (
                        float(i - kernel_height // 2 + (1 - kernel_height % 2)),
                    ...
                    current_weights = np.array(
                        weights[
                            i,
                            j,
                            current_start_feature : current_start_feature
                    ...
                    current_weights = np.pad(
                        current_weights,
                        [
                            [0, 4 - current_weights.shape[0]],
                            [0, 4 - current_weights.shape[1]],
                        ],
                    )
                    ...
                    if kernel_height == 1 and kernel_width == 1:
                        shader_string += (
                            "mat4" + str(current_weights) + " * g_" + str(k) + ";\n"
                        )
                    else:
                        shader_string += (
                            "mat4"
            ...
            if "lastresid" in layer.name:
                shader_string += (
                    "    return result + " + hook + "_tex(" + hook + "_pos);\n"
                )
            else:
...
def parse_add(layer, hook="MAIN", desc=None, when=None, max_bind_num=16):
    ...
    previous_layer_names = [
        format_layer_name(l) for l in layer._inbound_nodes[0].inbound_layers
    ]
    current_layer_name = format_layer_name(layer)
    ...
    for n in range(num_shaders):
        ...
        # Get bind name
        bind_names = [
            previous_layer_name + (str(n) if n > 0 else "")
        ...
        # Create hook body
        bind_str = [
            layer_name + "_tex(" + layer_name + "_pos)" for layer_name in bind_names
        ]
