...
function VideoSelect({ setVideoPath }: VideoSelectProps): JSX.Element {
  ...
  useDidUpdateEffect(() => {
    if (selectedFile) {
      ...
      convertVideo(selectedFile).then((convertedBlob: any) => {
        setTimeout(() => {
          uploadVideo(
            blobToFile(convertedBlob.data, selectedFile.name, selectedFile.size)
          )
            .then((res: any) => {
              console.log("video uploaded", res);
              setUploadMessage("Uploaded successfully");
              ipcRenderer.send("video-uploaded", res.data);
            })
            .catch(() => {
            ...
            .finally(() => {
              ...
              // remove later
              // ipcRenderer.send("video-uploaded", res);
            });
        }, 2000);
      });
    ...
    <div className="video-select-container">
      {selectedFile ? (
        <div
          className={
            isError
              ? "dashed-box dashed-box-error video-info flex-col"
              : "dashed-box video-info flex-col"
          }
        >
          ...
          <span className="flex" style={{ width: "100%" }}>
            <span className="flex" style={{ width: "100%" }}>
              <img
                src={isError ? uploadRed : uploadGrey}
                alt="upload"
                width="30"
              />
              &nbsp;&nbsp;&nbsp;
            ...
            <Button
              onClick={changeFile}
              appearance={isError ? "danger" : "default"}
            >
              Change
            </Button>
          </span>
